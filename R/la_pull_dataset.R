...
#'
#' Uses a dataset `key` or `id` from `la_list_datasets()` to pull data
...
#' metadata, dataset IDs are the more stable option.
#'
#' @param dataset A dataset key or ID from `la_list_datasets()`.
...
#' }
#' @export
la_pull_dataset <- function(dataset,
                             limit = 10000,
                             filters = list(),
                             date = NULL,
                             from = NULL,
  ...
  row <- cat_tbl[cat_tbl$key == dataset | cat_tbl$id == dataset, , drop = FALSE]

  if (nrow(row) == 0) {
    stop(
      paste0(
        "Unknown dataset: '", dataset, "'. ",
        "Use `la_list_datasets()` to see available keys and IDs."
      ),
      call. = FALSE
    )
  }
  ...
  if (nrow(row) > 1) {
    stop(
      paste0(
        ...
        "Please use the dataset ID instead."
      ),
      call. = FALSE
    )
  }
  ...
  date_where <- .la_build_date_where(
