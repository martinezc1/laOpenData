...
.la_postprocess <- function(df, clean_names = TRUE, coerce_types = TRUE) {
  ...
  if (isTRUE(coerce_types)) {
    df <- .la_coerce_types(df)
  }

  tibble::as_tibble(df, .name_repair = "minimal")
}

.la_validate_limit <- function(limit) {
  if (length(limit) != 1 || is.na(limit)) .la_abort("`limit` must be a single, non-missing numeric value.")
  if (!is.numeric(limit)) .la_abort("`limit` must be a single numeric value.")
  if (limit < 0) .la_abort("`limit` must be between 0 and Inf.")
  if (limit != floor(limit)) .la_abort("`limit` must be an integer (whole number).")
  as.integer(limit)
}

.la_validate_timeout <- function(timeout_sec) {
  if (length(timeout_sec) != 1 || is.na(timeout_sec)) {
    .la_abort("`timeout_sec` must be a single, non-missing numeric value.")
  }
  if (!is.numeric(timeout_sec)) {
    .la_abort("`timeout_sec` must be a single numeric value.")
  }
  if (timeout_sec <= 0) {
    .la_abort("`timeout_sec` must be > 0.")
  }
  timeout_sec
}

.la_validate_filters <- function(filters) {
  if (is.null(filters) || length(filters) == 0) return(list())

  if (!is.list(filters)) {
    .la_abort("`filters` must be a named list.")
  }
  if (is.null(names(filters)) || any(!nzchar(names(filters)))) {
    .la_abort("`filters` must be a *named* list (all elements must have names).")
  }
  if (any(vapply(filters, function(x) length(x) == 0, logical(1)))) {
    .la_abort("`filters` values cannot be empty.")
  }
  if (anyNA(unlist(filters))) {
    .la_abort("`filters` cannot contain NA values.")
  }

  filters
}

.la_endpoint <- function(dataset_id) {
  stopifnot(
    is.character(dataset_id),
    length(dataset_id) == 1,
    nzchar(dataset_id)
  )
  paste0("https://data.lacity.org/resource/", dataset_id, ".json")
}

.la_add_filters <- function(query, filters) {
  ...
  make_clause <- function(field, value) {
    ...
    if (is_text && any(!nzchar(value))) {
      .la_abort("`filters` character values cannot be empty or whitespace-only.")
    }
...
.la_add_where <- function(query, where) {
  if (is.null(where) || length(where) == 0) return(query)

  if (!is.character(where) || length(where) != 1 || is.na(where)) {
    .la_abort("`where` must be a single non-missing character string.")
  }
...
.la_get_json <- function(endpoint, query, timeout_sec = 30) {
  ...
  if (httr::http_error(resp)) {
    ...
    stop(
      paste0(
        "Los Angeles Open Data request failed with HTTP status ", status, ".\n",
...
.la_dataset_request <- function(dataset_id,
                                 limit = 10000,
                                 filters = list(),
                                 order = NULL,
                                 where = NULL,
  ...
  filters <- .la_validate_filters(filters)

  endpoint <- .la_endpoint(dataset_id)

  query_list <- list("$limit" = limit)

  if (!is.null(order)) {
    if (!is.character(order) || length(order) != 1 || is.na(order) || !nzchar(order)) {
      .la_abort("`order` must be a single non-empty character string.")
    }
    query_list[["$order"]] <- order
  }

  query_list <- .la_add_filters(query_list, filters)
  query_list <- .la_add_where(query_list, where)

  data <- .la_get_json(endpoint, query_list, timeout_sec = timeout_sec)
...
.la_validate_date_yyyy_mm_dd <- function(x, arg) {
  if (is.null(x)) return(NULL)
  if (inherits(x, "Date")) return(format(x, "%Y-%m-%d"))

  if (!is.character(x) || length(x) != 1 || is.na(x) || !nzchar(x)) {
    .la_abort(paste0("`", arg, "` must be a Date or a non-missing 'YYYY-MM-DD' string."))
  }
  if (!grepl("^\\d{4}-\\d{2}-\\d{2}$", x)) {
    .la_abort(paste0("`", arg, "` must be in 'YYYY-MM-DD' format."))
  }
  x
}

# Build a SoQL WHERE clause for date filtering using the dataset's date field
.la_build_date_where <- function(date_field, date = NULL, from = NULL, to = NULL) {
  if (is.null(date_field) || is.na(date_field) || !nzchar(date_field)) return(NULL)

  date <- .la_validate_date_yyyy_mm_dd(date, "date")
  from <- .la_validate_date_yyyy_mm_dd(from, "from")
  to   <- .la_validate_date_yyyy_mm_dd(to, "to")

  if (!is.null(date) && (!is.null(from) || !is.null(to))) {
    .la_abort("Use either `date` OR `from`/`to`, not both.")
  }
