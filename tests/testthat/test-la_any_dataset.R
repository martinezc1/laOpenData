test_that("la_any_dataset returns a tibble, respects limits, and supports postprocess toggles", {
  skip_if_not_installed("vcr")
  skip_if_not_installed("curl")

  endpoint <- "https://data.lacity.org/resource/29fd-3paw.json"

  vcr::use_cassette("la_any_dataset_robust", {
    base <- la_any_dataset(endpoint, limit = 2)
    ...
    raw_names <- la_any_dataset(endpoint, limit = 2, clean_names = FALSE, coerce_types = FALSE)
